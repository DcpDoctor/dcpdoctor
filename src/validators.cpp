  ...
  for(const auto& cpl_path : cpl_paths)
  {
    ...
    if(!key_ids.empty() || !enc_keys.empty())
    {
      notes.push_back({Severity::info, Code::encryption_detected,
                       "CPL contains encrypted content (" + std::to_string(key_ids.size()) +
                           " encrypted assets)",
                       cpl_path});
      ...
      for(auto& entry : fs::directory_iterator(dcp_dir, ec))
      {
        ...
        if(fname.find("KDM") != std::string::npos || fname.find("kdm") != std::string::npos)
        {
          kdm_found = true;
          break;
        }
      ...
      if(!kdm_found)
      {
        notes.push_back({Severity::info, Code::kdm_required,
                         "No KDM file found in DCP directory for encrypted content", cpl_path});
      }
    }
...
std::vector<Note> check_reel_continuity(const std::filesystem::path& cpl_path)
{
  ...
  for(auto reel_node : reels)
  {
    ...
    if(!entry_str.empty())
      entry_point = std::stoull(entry_str);
    if(!duration_str.empty())
      duration = std::stoull(duration_str);

    // Check entry point matches expected
    if(reel_num > 1 && entry_point != 0 && entry_point != expected_entry)
    {
      notes.push_back(
          {Severity::warning, Code::reel_discontinuity,
           "Reel " + std::to_string(reel_num) + " EntryPoint " + std::to_string(entry_point) +
               " does not follow previous reel (expected " + std::to_string(expected_entry) + ")",
           cpl_path});
    }
...
std::vector<Note> check_stereo(const std::filesystem::path& cpl_path)
{
  ...
  for(size_t i = 0; i < stereo.size(); ++i)
  {
    ...
    if(!left_eyes.empty() || !right_eyes.empty())
    {
      ...
      if(left_eyes.empty())
      {
        notes.push_back({Severity::error, Code::stereo_mismatch,
                         "Stereoscopic reel " + std::to_string(i + 1) + " missing LeftEye",
                         cpl_path});
      }
      if(right_eyes.empty())
      {
        notes.push_back({Severity::error, Code::stereo_mismatch,
                         "Stereoscopic reel " + std::to_string(i + 1) + " missing RightEye",
                         cpl_path});
      }
    }
    ...
    if(!duration.empty() && !intrinsic.empty())
    {
      ...
      {
        notes.push_back(
            {Severity::error, Code::stereo_mismatch,
             "Stereoscopic reel " + std::to_string(i + 1) + " Duration exceeds IntrinsicDuration",
             cpl_path});
      }
    }
  }
    ...
    for(const auto& [label, desc] : required_markers)
    {
      if(!found_markers.contains(label))
      {
        notes.push_back({Severity::warning, Code::marker_missing,
                         "Required marker missing: " + label + " (" + desc + ")", cpl_path});
      }
    }

    // Recommended markers
    static const std::vector<std::pair<std::string, std::string>> recommended_markers = {
        {"FFTC", "First Frame of Title Credits"}, {"LFTC", "Last Frame of Title Credits"},
        {"FFOI", "First Frame of Intermission"},  {"LFOI", "Last Frame of Intermission"},
        {"FFEC", "First Frame of End Credits"},   {"LFEC", "Last Frame of End Credits"},
    };
    ...
    for(const auto& [label, desc] : recommended_markers)
    {
      if(!found_markers.contains(label))
      {
        notes.push_back({Severity::info, Code::marker_missing,
                         "Recommended marker not present: " + label + " (" + desc + ")", cpl_path});
      }
    }
  }
                                         ...
                                         const std::vector<std::filesystem::path>& cpl_paths)
{
  ...
  for(const auto& cpl_path : cpl_paths)
  {
    ...
    const char* asset_id_elements[] = {
        "MainPicture", "MainSound",     "MainSubtitle", "MainStereoscopicPicture",
        "MainMarkers", "ClosedCaption", "MainImage",    "AuxData"};
        ...
        if(!id.empty())
        {
          ...
          if(!known_ids.contains(id))
          {
            notes.push_back({Severity::error, Code::cross_ref_broken,
                             "CPL references asset " + id + " not found in ASSETMAP/PKL",
                             cpl_path});
          }
        }
      }
  ...
  for(const auto& cpl_path : cpl_paths)
  {
    ...
    if(!ref_cpls.empty() || !opl_nodes.empty())
    {
      ...
      notes.push_back({Severity::info, Code::supplemental_opl_missing,
                       "CPL appears to be a supplemental/version file package", cpl_path});
    }
...
std::vector<Note> check_audio_channels(const std::filesystem::path& cpl_path)
{
  ...
  for(size_t i = 0; i < sounds.size(); ++i)
  {
    ...
    if(mca.empty())
    {
      ...
      {
        notes.push_back(
            {Severity::info, Code::sound_invalid_channel_count,
             "Reel " + std::to_string(i + 1) + " sound has no MCA channel labeling metadata",
             cpl_path});
      }
    }
  }
...
std::vector<Note> check_color_space(const std::filesystem::path& cpl_path)
{
  ...
  for(auto tc : tc_nodes)
  {
    ...
    if(content)
    {
      ...
      if(val.find("2.6") == std::string::npos && val.find("XYZ") == std::string::npos)
      {
        notes.push_back({Severity::info, Code::picture_invalid_resolution,
                         "TransferCharacteristic indicates non-DCI color: " + val, cpl_path});
      }
    }
  }
