...
DkdmInfo parse_dkdm(const fs::path& dkdm_path)
{
  ...
  if(!not_before.empty())
    info.not_valid_before = parse_iso_time(not_before);
  if(!not_after.empty())
    info.not_valid_after = parse_iso_time(not_after);
...
std::vector<Note> validate_dkdm(const fs::path& dkdm_path)
{
  ...
  {
    notes.push_back(
        Note{Severity::error, Code::encryption_detected, "Invalid DKDM: " + info.error, dkdm_path});
    return notes;
  }
  ...
  if(!info.is_dkdm)
  {
    notes.push_back(Note{Severity::info, Code::encryption_detected,
                         "File is a KDM (not a DKDM) - has DeviceListIdentifier", dkdm_path});
  }
  ...
  {
    notes.push_back(
        Note{Severity::error, Code::encryption_detected, "DKDM has expired", dkdm_path});
  }
...
std::vector<TdlEntry> load_trusted_device_list(const fs::path& tdl_path)
{
  ...
  for(auto td = root->children; td; td = td->next)
  {
    ...
    if(xmlStrcmp(td->name, BAD_CAST "TrustedDevice") != 0 &&
       xmlStrcmp(td->name, BAD_CAST "Device") != 0)
      continue;
                                           ...
                                           const std::vector<TdlEntry>& tdl)
{
  ...
  {
    if(entry.thumbprint == recipient_serial || entry.common_name == recipient_serial)
    {
      found = true;
      break;
    }
  ...
  {
    notes.push_back(Note{Severity::warning, Code::encryption_detected,
                         "KDM recipient not found in Trusted Device List", kdm_path});
  }
