...
std::vector<uint8_t> canonicalize_subtree(xmlDocPtr doc, xmlNodePtr node, int mode) {
    ...
    for (auto ancestor = node->parent; ancestor; ancestor = ancestor->parent) {
        if (ancestor->type != XML_ELEMENT_NODE) continue;
...
std::vector<Note> verify_signature(const std::filesystem::path& xml_file) {
    ...
    if (!signed_info) {
        notes.push_back({Severity::error, Code::signature_invalid,
                         "Signature missing SignedInfo", xml_file});
        xmlFreeDoc(doc);
        return notes;
    }
    ...
    if (!sig_value_node) {
        notes.push_back({Severity::error, Code::signature_invalid,
                         "Signature missing SignatureValue", xml_file});
        xmlFreeDoc(doc);
        return notes;
    }
    ...
    if (sig_value.empty()) {
        notes.push_back({Severity::error, Code::signature_invalid,
                         "Cannot decode SignatureValue", xml_file});
        xmlFreeDoc(doc);
        return notes;
    }
    ...
    if (!cert_node) {
        notes.push_back({Severity::warning, Code::signature_invalid,
                         "No X509Certificate in signature", xml_file});
        xmlFreeDoc(doc);
        return notes;
    }
    ...
    if (!cert) {
        notes.push_back({Severity::error, Code::signature_invalid,
                         "Cannot parse X509 certificate", xml_file});
        xmlFreeDoc(doc);
        return notes;
    }
    ...
    int c14n_mode = (c14n_uri.find("exc-c14n") != std::string::npos)
                        ? XML_C14N_EXCLUSIVE_1_0 : XML_C14N_1_0;
    ...
    if (signed_info_c14n.empty()) {
        notes.push_back({Severity::error, Code::signature_invalid,
                         "Failed to canonicalize SignedInfo", xml_file});
        X509_free(cert);
        xmlFreeDoc(doc);
        return notes;
    }
    ...
    if (pkey) {
        ...
        bool sig_ok = (EVP_DigestVerifyInit(md_ctx, nullptr, sig_md, nullptr, pkey) == 1 &&
        ...
        if (!sig_ok) {
            notes.push_back({Severity::error, Code::signature_invalid,
                             "Signature verification failed", xml_file});
        }
    }

    // Verify Reference digests
    for (auto ref = signed_info->children; ref; ref = ref->next) {
        if (ref->type != XML_ELEMENT_NODE) continue;
        if (std::strcmp(reinterpret_cast<const char*>(ref->name), "Reference") != 0) continue;
        ...
        if (!digest_value_node) continue;

        const EVP_MD* ref_md = get_digest_from_uri(
            digest_method_node ? get_attr(digest_method_node, "Algorithm") : "");
        auto expected_digest = base64_decode(get_text(digest_value_node));
        if (expected_digest.empty()) continue;
        ...
        if (uri.empty() || uri[0] == '#') {
            ...
            if (uri.empty()) {
                ...
                if (xctx) {
                    ...
                    if (xobj) xmlXPathFreeObject(xobj);
...
std::vector<Note> verify_certificate_chain(const std::filesystem::path& xml_file) {
    ...
    auto sig_node = find_child_recursive(root, "Signature");
    if (!sig_node) { xmlFreeDoc(doc); return notes; }

    auto key_info = find_child(sig_node, "KeyInfo");
    if (!key_info) { xmlFreeDoc(doc); return notes; }
    ...
    if (certs.size() >= 2) {
        ...
        if (X509_verify_cert(ctx) != 1) {
            ...
            notes.push_back({Severity::error, Code::certificate_chain_broken,
                             std::string("Certificate chain invalid: ") +
                             X509_verify_cert_error_string(err), xml_file});
        }
