...
std::vector<TheaterProfile> get_theater_profiles()
{
  return {{"Dolby IMS3000",
           ...
           {"Requires BV2.1 for Atmos content", "May reject DCPs with non-standard MCA labels",
          ...
          {"Barco SP4K",
           ...
           16,
           500,
           {"Requires SMPTE standard for 4K", "No native Dolby Atmos (requires external processor)",
          ...
          {"Barco SP2K",
           ...
           8,
           250,
           {"Limited to 2K projection", "May not support all SMPTE subtitle features"}},
          {"Christie CP4440-RGB",
           "Christie",
           true,
           true,
           ...
           16,
           500,
           {"No native Atmos support", "Requires external IMB for content decryption",
          ...
          {"Christie CP2230",
           ...
           8,
           250,
           {"Legacy 2K DLP projection", "Limited subtitle font support", "No HFR capability"}},
          {"GDC SX-4000",
           "GDC",
           true,
           true,
          ...
          {"GDC SR-1000",
           ...
           {"Legacy server - limited feature support",
            "May have issues with large CPLs (>50 reels)", "No subtitle font embedding support"}},
          {"Dolby Cinema (Premium)",
           ...
           128, // Atmos objects
           500,
           {"SMPTE-only (rejects Interop)", "Requires BV2.1 compliance",
          ...
          {"IMAX Digital",
           ...
           12, // IMAX 12-channel
           500,
           {"SMPTE-only", "Requires IMAX-specific audio channel layout",
...
const TheaterProfile* find_profile(const std::string& query)
{
  ...
  for(const auto& profile : profiles)
  {
    ...
    for(char c : profile.vendor)
      lower_vendor += std::tolower(c);
...
std::vector<Note> check_theater_compatibility(const fs::path& dcp_dir, const VerifyResult& result,
                                              const TheaterProfile& profile)
{
  ...
  if(!profile.supports_interop && result.standard == Standard::interop)
  {
    notes.push_back(Note{Severity::error, Code::smpte_namespace_wrong,
                         profile.name + " does not support Interop standard", dcp_dir});
  }
  ...
  if(profile.requires_bv21 && result.standard == Standard::smpte)
  {
    ...
    if(!fs::exists(dcp_dir / "ASSETMAP.xml"))
    {
      notes.push_back(Note{Severity::warning, Code::smpte_naming_violation,
                           profile.name + " requires BV2.1 (ASSETMAP.xml naming)", dcp_dir});
    }
  }
