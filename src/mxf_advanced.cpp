...
static const uint8_t partition_pack_key[] = {0x06, 0x0e, 0x2b, 0x34, 0x02, 0x05, 0x01, 0x01,
...
std::vector<Note> check_mxf_partitions(const MxfPartitionInfo& info, const fs::path& mxf_path)
{
  ...
  if(!info.closed_complete)
  {
    notes.push_back(Note{Severity::info, Code::mxf_invalid_structure,
                         "MXF header partition not Closed & Complete", mxf_path});
  }
...
std::vector<Note> check_dtsx_compliance(const DtsxInfo& info, const fs::path& mxf_path)
{
  ...
  notes.push_back(
      Note{Severity::info, Code::sound_invalid_channel_count,
           "DTS:X Immersive Audio detected (" + std::to_string(info.channel_count) + " channels)",
           mxf_path});
