...
namespace
{
  ...
  constexpr std::array<uint8_t, 8> RGBA_DESCRIPTOR_UL = {0x02, 0x53, 0x00, 0x00,
                                                         0x0d, 0x01, 0x01, 0x01};
  ...
  {
    if(std::memcmp(ul.data(), UL_PREFIX.data(), 4) != 0)
      return false;
    if(ul[4] != 0x02 || ul[5] != 0x53)
      return false;
  ...
  EssenceType detect_essence_type(const UL& ec_label)
  {
    if(is_jpeg2000_container(ec_label))
      return EssenceType::jpeg2000;
    if(is_pcm_container(ec_label))
      return EssenceType::pcm_audio;
    if(is_mpeg2_container(ec_label))
      return EssenceType::mpeg2;
    if(is_timed_text_container(ec_label))
      return EssenceType::timed_text;
    if(is_dolby_atmos_container(ec_label))
      return EssenceType::dolby_atmos;
  ...
  while(f && scanned < MAX_SCAN)
  {
    ...
    if(is_picture_descriptor_ul(klv_key) && klv_len <= 65536)
    {
      ...
      if(f)
      {
        ...
        pic.type =
            (info.essence_type == EssenceType::unknown) ? EssenceType::jpeg2000 : info.essence_type;
...
std::string_view essence_type_str(EssenceType t)
{
  switch(t)
  {
    case EssenceType::jpeg2000:
      return "JPEG 2000";
    case EssenceType::mpeg2:
      return "MPEG-2";
    case EssenceType::pcm_audio:
      return "PCM Audio";
    case EssenceType::timed_text:
      return "Timed Text";
    case EssenceType::dolby_atmos:
      return "Dolby Atmos (IAB)";
    default:
      return "Unknown";
  }
}
