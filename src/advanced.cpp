    ...
    if(fname.find("PKL") != std::string::npos || fname.find("pkl") != std::string::npos)
    {
      if(entry.path().extension() != ".xml")
      {
        notes.push_back({Severity::warning, Code::smpte_naming_violation,
                         "BV2.1: PKL file should have .xml extension: " + fname, entry.path()});
      }
    }
  }
...
std::vector<Note> compare_manifest(const fs::path& dcp_dir, const fs::path& manifest_path)
{
  std::vector<Note> notes;
  ...
  if(!f)
  {
    notes.push_back({Severity::error, Code::asset_not_found,
                     "Cannot open manifest file: " + manifest_path.string(), manifest_path});
    return notes;
  }
  ...
  while((pos = content.find("\"filename\"", pos)) != std::string::npos)
  {
    ...
    {
      notes.push_back({Severity::error, Code::asset_not_found,
                       "Manifest asset not found in DCP: " + filename, dcp_dir});
    }
          ...
          if(fs::exists(full_path))
          {
            ...
            if(actual_size != expected_size)
            {
              notes.push_back({Severity::error, Code::pkl_hash_mismatch,
                               "Size mismatch for " + filename + ": expected " +
  ...
  for(auto& entry : fs::directory_iterator(dcp_dir, ec))
  {
    ...
    if(!manifest_files.contains(fname))
    {
      notes.push_back(Note{Severity::info, Code::asset_not_found,
                           "File in DCP not listed in manifest: " + fname, entry.path()});
    }
  }
...
void write_batch_summary(std::ostream& out, const std::vector<BatchResult>& results)
{
  ...
  out << "Total: " << total << "  Passed: " << passed << "  Failed: " << (total - passed) << "\n\n";

  // Table header
  out << std::left << std::setw(50) << "DCP Path" << std::setw(10) << "Status" << std::setw(10)
      << "Errors" << std::setw(10) << "Warnings"
      << "Standard\n";
  out << std::string(90, '-') << "\n";
  ...
  for(const auto& r : results)
  {
    ...
    if(path_str.size() > 48)
      path_str = path_str.substr(0, 45) + "...";

    out << std::left << std::setw(50) << path_str << std::setw(10) << (r.passed ? "PASS" : "FAIL")
        << std::setw(10) << r.errors << std::setw(10) << r.warnings
        << (r.standard == Standard::smpte     ? "SMPTE"
            : r.standard == Standard::interop ? "Interop"
                                              : "Unknown")
        << "\n";
  }
}
