...
std::vector<Note> check_isdcf_naming(const std::string& content_title,
    ...
    if (content_title.empty()) {
        notes.push_back({Severity::warning, Code::isdcf_naming_violation,
                        "Content title is empty", cpl_path});
        return notes;
    }
    ...
    if (fields[0].empty()) {
        notes.push_back({Severity::warning, Code::isdcf_naming_violation,
                        "Film title field is empty", cpl_path});
    } else if (fields[0].size() > 14) {
        notes.push_back({Severity::info, Code::isdcf_naming_violation,
                        "Film title exceeds 14 character recommendation: " + fields[0],
                        cpl_path});
    }
    ...
    if (fields.size() >= 2) {
        static const std::set<std::string> valid_types = {
            "FTR", "TLR", "TSR", "PRO", "TST", "RTG", "SHR",
            "ADV", "XSN", "PSA", "POL", "CLT"
        };
        if (!valid_types.contains(fields[1])) {
            notes.push_back({Severity::warning, Code::isdcf_naming_violation,
                            "Non-standard content type: " + fields[1] +
                            " (expected FTR, TLR, TSR, PRO, TST, etc.)", cpl_path});
        }
    }
    ...
    if (fields.size() >= 3) {
        static const std::set<std::string> valid_aspects = {
            "F", "S", "C", "F-133", "F-137", "F-138", "F-165",
            "F-166", "F-178", "F-185", "F-190", "F-200", "F-220",
            "F-239", "S-185", "S-239", "C-185", "C-239"
        };
