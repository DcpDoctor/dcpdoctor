...
static const char* smpte_root_thumbprints[] = {
    ...
    "a178bdc1bd18c9d5f1e1cf12ec4cee71fbe50237", "44c8a37b7f3f5e4d9c3ba7e91dfd7d64a83c5c14",
    nullptr};
                                     ...
                                     double frame_rate, uint8_t bit_depth, uint8_t num_components)
{
  ...
  // Clamp
  if(score.estimated_psnr_db > 55.0)
    score.estimated_psnr_db = 55.0;
  if(score.estimated_psnr_db < 25.0)
    score.estimated_psnr_db = 25.0;
  if(score.quality_score > 100.0)
    score.quality_score = 100.0;

  std::ostringstream oss;
  oss << std::fixed << std::setprecision(2) << score.bits_per_pixel << " bpp, ~"
      << int(score.estimated_psnr_db) << " dB PSNR, " << std::setprecision(1)
      << score.compression_ratio << ":1 compression";
  score.notes = oss.str();
...
std::vector<Note> check_cert_chain_compliance(const CertChainInfo& info, const fs::path& dcp_dir)
{
  ...
  notes.push_back(Note{Severity::info, Code::signature_invalid,
                       "Certificate chain: " + std::to_string(info.chain_length) + " certificates",
                       dcp_dir});
