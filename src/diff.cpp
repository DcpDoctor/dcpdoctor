...
std::map<std::string, AssetEntry> parse_assetmap(const fs::path& dcp_dir) {
    ...
    fs::path am_path;
    if (fs::exists(dcp_dir / "ASSETMAP.xml")) am_path = dcp_dir / "ASSETMAP.xml";
    ...
    if (!root) { xmlFreeDoc(doc); return assets; }
    ...
    for (auto al = root->children; al; al = al->next) {
        ...
        for (auto asset = al->children; asset; asset = asset->next) {
            ...
            if (xmlStrcmp(asset->name, BAD_CAST "Asset") != 0) continue;
            ...
            for (auto cl = asset->children; cl; cl = cl->next) {
                ...
                if (xmlStrcmp(cl->name, BAD_CAST "ChunkList") != 0) continue;
                for (auto chunk = cl->children; chunk; chunk = chunk->next) {
                    if (chunk->type != XML_ELEMENT_NODE) continue;
                    if (xmlStrcmp(chunk->name, BAD_CAST "Chunk") != 0) continue;
                    entry.filename = get_text(chunk, "Path");
                    auto sz = get_text(chunk, "Length");
                    if (!sz.empty()) entry.size = std::stoull(sz);
                }
            }
...
DcpDiff compare_dcps(const fs::path& dcp_a, const fs::path& dcp_b, bool check_hashes) {
    ...
    for (auto& [id, _] : assets_a) all_ids.insert(id);
    for (auto& [id, _] : assets_b) all_ids.insert(id);
...
void write_diff_report(std::ostream& out, const DcpDiff& diff) {
    ...
    for (const auto& ad : diff.assets) {
        if (ad.status == DcpDiff::AssetDiff::Status::same) continue;

        const char* marker = "";
        switch (ad.status) {
        case DcpDiff::AssetDiff::Status::added: marker = "+"; break;
        case DcpDiff::AssetDiff::Status::removed: marker = "-"; break;
        case DcpDiff::AssetDiff::Status::modified: marker = "~"; break;
        default: break;
        }
        out << "  " << marker << " " << ad.filename;
        if (!ad.detail.empty()) out << " (" << ad.detail << ")";
        out << "\n";
    }
}
