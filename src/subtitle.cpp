    ...
    if(time_in && time_out)
    {
      ...
      if(valid_timecode(tin) && valid_timecode(tout) && tout <= tin)
      {
        ctx.notes.push_back({Severity::warning, Code::subtitle_invalid_timing,
                             "TimeOut (" + tout + ") not after TimeIn (" + tin + ")", ctx.path});
      }
    }
...
std::vector<Note> validate_subtitle(const std::filesystem::path& xml_path, Standard standard)
{
  ...
  {
    ctx.notes.push_back(
        {Severity::error, Code::subtitle_parse_error, "Failed to parse subtitle XML", xml_path});
    return ctx.notes;
  }
  ...
  {
    ctx.notes.push_back(
        {Severity::error, Code::subtitle_parse_error, "Empty subtitle document", xml_path});
    xmlFreeDoc(doc);
    return ctx.notes;
  }
  ...
  {
    ctx.notes.push_back(
        {Severity::error, Code::subtitle_parse_error,
         "Unexpected root element: " + root_name + " (expected SubtitleReel or DCSubtitle)",
         xml_path});
    xmlFreeDoc(doc);
    return ctx.notes;
  }
  ...
  for(auto child = root->children; child; child = child->next)
  {
    ...
    if(cname == "Id" || cname == "SubtitleID")
      has_id = true;
    if(cname == "EditRate")
      has_edit_rate = true;
    if(cname == "TimeCodeRate")
      has_time_code_rate = true;
  }

  if(!has_id)
  {
    ctx.notes.push_back(
        {Severity::error, Code::missing_required_element, "Subtitle missing Id element", xml_path});
  }
