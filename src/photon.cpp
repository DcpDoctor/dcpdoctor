...
bool build_photon(const PhotonConfig& config)
{
  ...
  std::error_code ec;
  if(!fs::exists(config.photon_dir / "gradlew", ec))
    return false;

  // Run gradle build
  std::string cmd =
      "cd " + config.photon_dir.string() + " && ./gradlew build getDependencies -x test 2>&1";
...
namespace
{
  ...
  {
    if(level_str == "FATAL")
      return PhotonError::Level::fatal;
    if(level_str == "WARNING")
      return PhotonError::Level::warning;
...
PhotonResult run_photon(const fs::path& imf_dir, const PhotonConfig& config)
{
  ...
  // Construct command
  std::string cmd = config.java_executable.string() + " -cp \"" + classpath + ":\"" +
                    " com.netflix.imflibrary.app.IMPAnalyzer" + " \"" + imf_dir.string() + "\"" +
                    " 2>&1";
...
std::vector<Note> photon_to_notes(const PhotonResult& result, const fs::path& imf_dir)
{
  ...
  if(!result.success)
  {
    notes.push_back(Note{Severity::warning, Code::missing_assetmap,
                         "Photon IMF validation unavailable: " + result.error_message, imf_dir});
    return notes;
  }
  ...
  if(result.errors.empty())
  {
    notes.push_back(Note{Severity::info, Code::missing_assetmap,
                         "Photon: IMF package validates cleanly (no errors)", imf_dir});
    return notes;
  }
  ...
  for(const auto& err : result.errors)
  {
    ...
    switch(err.level)
    {
      case PhotonError::Level::fatal:
        sev = Severity::error;
        break;
      case PhotonError::Level::error:
        sev = Severity::error;
        break;
      case PhotonError::Level::warning:
    ...
    if(err.code == "IMF_AM_ERROR")
      code = Code::missing_assetmap;
    else if(err.code == "IMF_PKL_ERROR")
      code = Code::missing_pkl;
    else if(err.code == "IMF_CPL_ERROR")
      code = Code::missing_cpl;
    else if(err.code == "IMF_ESSENCE_ERROR")
      code = Code::mxf_invalid_structure;

    fs::path file_path = err.file.empty() ? imf_dir : fs::path(err.file);
    notes.push_back(Note{sev, code, "[Photon] " + err.description, file_path});
  }
