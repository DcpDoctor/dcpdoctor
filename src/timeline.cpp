...
std::vector<TimelineReel> extract_timeline(const std::filesystem::path& cpl_path) {
    ...
    if (!root) { xmlFreeDoc(doc); return reels; }

    // Find ReelList
    for (auto rl = root->children; rl; rl = rl->next) {
        if (rl->type != XML_ELEMENT_NODE) continue;
        if (xmlStrcmp(rl->name, BAD_CAST "ReelList") != 0) continue;

        for (auto reel_node = rl->children; reel_node; reel_node = reel_node->next) {
            if (reel_node->type != XML_ELEMENT_NODE) continue;
            if (xmlStrcmp(reel_node->name, BAD_CAST "Reel") != 0) continue;
            ...
            for (auto al = reel_node->children; al; al = al->next) {
                if (al->type != XML_ELEMENT_NODE) continue;
                if (xmlStrcmp(al->name, BAD_CAST "AssetList") != 0) continue;
                ...
                for (auto asset = al->children; asset; asset = asset->next) {
                    ...
                    if (name == "MainPicture" || name == "MainImage" ||
                        name == "MainStereoscopicPicture") {
                        reel.has_picture = true;
                        auto ep = get_text(asset, "EntryPoint");
                        auto dur = get_text(asset, "Duration");
                        if (!ep.empty()) reel.picture_entry = std::stoull(ep);
                        if (!dur.empty()) reel.picture_duration = std::stoull(dur);
                    } else if (name == "MainSound") {
                        reel.has_sound = true;
                        auto ep = get_text(asset, "EntryPoint");
                        auto dur = get_text(asset, "Duration");
                        if (!ep.empty()) reel.sound_entry = std::stoull(ep);
                        if (!dur.empty()) reel.sound_duration = std::stoull(dur);
                    } else if (name == "MainSubtitle" || name == "ClosedCaption") {
                        reel.has_subtitle = true;
                        auto ep = get_text(asset, "EntryPoint");
                        auto dur = get_text(asset, "Duration");
                        if (!ep.empty()) reel.subtitle_entry = std::stoull(ep);
                        if (!dur.empty()) reel.subtitle_duration = std::stoull(dur);
                    }
                }
            }
...
void write_timeline_svg(std::ostream& out,
                        const std::vector<TimelineReel>& reels,
...
<style>
  ...
  .reel-line { stroke: #ddd; stroke-width: 1; stroke-dasharray: 4; }
    ...
    // Title
    out << "<text x=\"" << margin << "\" y=\"30\" class=\"title\">"
        << title << " (" << format_timecode(total_frames, frame_rate) << ")</text>\n";
    ...
    out << "<text x=\"5\" y=\"" << y_pic + track_height/2 + 4 << "\" class=\"label\">Picture</text>\n";
    out << "<text x=\"5\" y=\"" << y_snd + track_height/2 + 4 << "\" class=\"label\">Sound</text>\n";
    out << "<text x=\"5\" y=\"" << y_sub + track_height/2 + 4 << "\" class=\"label\">Subtitle</text>\n";
    ...
    for (size_t i = 0; i < reels.size(); ++i) {
        ...
        // Reel separator
        if (i > 0) {
            out << "<line x1=\"" << int(x) << "\" y1=\"" << header_height
                << "\" x2=\"" << int(x) << "\" y2=\"" << height - 20
                << "\" class=\"reel-line\"/>\n";
        }
        ...
        out << "<text x=\"" << int(x + 2) << "\" y=\"" << height - 5
            << "\" class=\"tc\">" << format_timecode(offset, frame_rate) << "</text>\n";

        offset += r.picture_duration;
    }

    // End timecode
    out << "<text x=\"" << int(margin + total_frames * pixels_per_frame - 60) << "\" y=\"" << height - 5
        << "\" class=\"tc\">" << format_timecode(total_frames, frame_rate) << "</text>\n";
