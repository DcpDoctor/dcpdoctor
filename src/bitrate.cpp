...
FrameBitrateStats analyze_picture_bitrate(const std::filesystem::path& mxf_path)
{
  ...
  stats.max_bitrate_mbps = (double(stats.max_frame_bytes) * 8.0) / (frame_duration_sec * 1000000.0);
  stats.min_bitrate_mbps = (double(stats.min_frame_bytes) * 8.0) / (frame_duration_sec * 1000000.0);
                                           ...
                                           const std::filesystem::path& mxf_path)
{
  ...
  else if(stats.max_bitrate_mbps > max_allowed * 0.95)
  {
    notes.push_back({Severity::warning, Code::j2k_bitrate_exceeded,
                     "Peak frame bitrate " + std::to_string(int(stats.max_bitrate_mbps)) +
                         " Mbps is near DCI limit of " + std::to_string(int(max_allowed)) + " Mbps",
                     mxf_path});
  }
                                            ...
                                            const std::filesystem::path& mxf_path)
{
  ...
  {
    notes.push_back(
        {Severity::error, Code::j2k_invalid_component_count,
         "J2K has " + std::to_string(info.num_components) + " components (DCI requires 3 for XYZ)",
         mxf_path});
  }
  ...
  if(info.bit_depth != 12 && info.bit_depth != 0)
  {
    notes.push_back({Severity::warning, Code::j2k_invalid_profile,
                     "J2K bit depth is " + std::to_string(info.bit_depth) + " (DCI standard is 12)",
                     mxf_path});
  }
  ...
  if(info.rsiz != RSIZ_CINEMA_2K && info.rsiz != RSIZ_CINEMA_4K &&
     info.rsiz != RSIZ_CINEMA_2K_S3D && info.rsiz != RSIZ_CINEMA_4K_S3D && info.rsiz != 0)
  {
    notes.push_back(
        {Severity::info, Code::j2k_invalid_profile,
         "J2K profile RSIZ=" + std::to_string(info.rsiz) + " is not a Cinema profile (3/4/5/6)",
         mxf_path});
  }
  ...
  if(cb_w > 0 && cb_h > 0)
  {
    ...
    if(actual_w != 32 && actual_w != 64)
    {
      notes.push_back({Severity::info, Code::j2k_invalid_profile,
                       "Code-block width " + std::to_string(actual_w) + " (DCI standard: 32 or 64)",
                       mxf_path});
    }
  }
  ...
  if(info.num_decomp_levels > 0 && info.num_decomp_levels < 5)
  {
    notes.push_back({Severity::warning, Code::j2k_invalid_profile,
                     "Only " + std::to_string(info.num_decomp_levels) +
                         " decomposition levels (DCI recommends >= 5)",
                     mxf_path});
  }
