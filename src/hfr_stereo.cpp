...
std::vector<Note> check_hfr_compliance(const fs::path& cpl_path) {
    ...
    if (fps > 30.0) {
        ...
        notes.push_back(Note{Severity::info, Code::cpl_invalid_edit_rate,
                        "HFR content detected: " + std::to_string(int(fps)) + " fps",
                        cpl_path});
...
MultiCplInfo analyze_multi_cpl(const fs::path& dcp_dir) {
    ...
    for (auto& entry : fs::directory_iterator(dcp_dir, ec)) {
        ...
        for (auto child = root->children; child; child = child->next) {
            ...
            if (xmlStrcmp(child->name, BAD_CAST "ReelList") != 0) continue;
            for (auto reel = child->children; reel; reel = reel->next) {
                if (reel->type == XML_ELEMENT_NODE &&
                    xmlStrcmp(reel->name, BAD_CAST "Reel") == 0)
                    cpl.reel_count++;
            }
        }
        ...
        for (char c : cpl.content_title) title_lower += std::tolower(c);
        if (title_lower.find("trailer") != std::string::npos) cpl.type = "trailer";
        else if (title_lower.find("advert") != std::string::npos) cpl.type = "advertisement";
        else if (title_lower.find("test") != std::string::npos) cpl.type = "test";
        else cpl.type = "main";
...
Stereo3dInfo analyze_stereo3d(const fs::path& cpl_path) {
    ...
    auto root = xmlDocGetRootElement(doc);
    if (!root) { xmlFreeDoc(doc); return info; }
    ...
    if (info.is_stereo) {
        info.eye_offset = static_cast<int64_t>(info.left_duration) -
                         static_cast<int64_t>(info.right_duration);
...
std::vector<Note> check_stereo3d_compliance(const Stereo3dInfo& info,
                                             const fs::path& cpl_path) {
    std::vector<Note> notes;
    if (!info.is_stereo) return notes;
    ...
    if (!info.eyes_aligned) {
        notes.push_back(Note{Severity::error, Code::stereo_mismatch,
                        ...
                        " frame offset between left and right eyes", cpl_path});
    }
    ...
    notes.push_back(Note{Severity::info, Code::stereo_mismatch,
                    "3D stereoscopic content (" + info.stereo_type + "): L=" +
                    std::to_string(info.left_duration) + " R=" +
                    std::to_string(info.right_duration) + " frames", cpl_path});
...
std::vector<Note> check_cpl_chain(const std::vector<CplChainEntry>& chain,
    ...
    for (const auto& entry : chain) {
        if (entry.is_supplemental) supplemental_count++;
    }
    ...
    if (supplemental_count > 0) {
        notes.push_back(Note{Severity::info, Code::supplemental_opl_missing,
                        ...
                        " supplemental CPL(s) in version chain", dcp_dir});
            ...
            if (entry.is_supplemental && !entry.original_cpl_id.empty()) {
                if (!known_ids.contains(entry.original_cpl_id)) {
                    notes.push_back(Note{Severity::warning, Code::supplemental_opl_missing,
                                    "Supplemental CPL references original " +
                                    entry.original_cpl_id + " not found in this package",
                                    dcp_dir});
                }
            }
        }
