...
std::vector<Note> validate_kdm(const fs::path& kdm_path, const fs::path& dcp_dir) {
    ...
    if (!info.valid) {
        notes.push_back(Note{Severity::error, Code::encryption_detected,
                        "Invalid KDM: " + info.error, kdm_path});
        return notes;
    }

    if (info.is_expired) {
        notes.push_back(Note{Severity::error, Code::encryption_detected,
                        "KDM has expired", kdm_path});
    }
    ...
    if (!info.cpl_id.empty() && fs::is_directory(dcp_dir)) {
        ...
        for (auto& entry : fs::directory_iterator(dcp_dir, ec)) {
            ...
            if (root) {
                ...
                if (rn == "CompositionPlaylist") {
                    ...
                    if (id.starts_with("urn:uuid:")) id = id.substr(9);
                    std::string cpl_ref = info.cpl_id;
                    if (cpl_ref.starts_with("urn:uuid:")) cpl_ref = cpl_ref.substr(9);
                    if (id == cpl_ref)
                        found_cpl = true;
                }
            }
        ...
        if (!found_cpl) {
            notes.push_back(Note{Severity::warning, Code::cross_ref_broken,
                            "KDM references CPL " + info.cpl_id +
                            " which was not found in the DCP", kdm_path});
        }
    }
