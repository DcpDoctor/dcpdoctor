...
static constexpr std::string_view interop_cpl_ns = "http://www.digicine.com/PROTO-ASDCP-CPL-20040511#";
...
VerifyResult verify(const fs::path& dcp_dir, const VerifyOptions& opts) {
    ...
    if (assetmap_path.empty()) {
        result.add({Severity::error, Code::missing_assetmap,
                    "No ASSETMAP or ASSETMAP.xml found", dcp_dir});
        return result;
    }

    auto assetmap = AssetMap::parse(assetmap_path);
    if (!assetmap) {
        result.add({Severity::error, Code::xml_parse_error,
                    "Failed to parse ASSETMAP", assetmap_path});
        return result;
    }
    ...
    for (const auto& asset : assetmap->assets) {
        if (!seen_ids.insert(asset.id).second) {
            result.add({Severity::error, Code::duplicate_asset_id,
                        "Duplicate asset ID: " + asset.id, assetmap_path});
        }
    }
    ...
    for (const auto& asset : assetmap->assets) {
        ...
        if (!fs::exists(full_path)) {
            result.add({Severity::error, Code::asset_not_found,
                        "Asset file not found: " + asset.path, assetmap_path});
        }
    }
                    ...
                    if (fs::exists(asset_path)) {
                        ...
                        if (computed && *computed != pkl_asset.hash) {
                            result.add({Severity::error, Code::pkl_hash_mismatch,
                                        "Hash mismatch for " + it->second +
                                        " (expected " + pkl_asset.hash +
                                        ", got " + *computed + ")",
                                        asset_path});
                        }
                    }
    ...
    if (!found_pkl) {
        result.add({Severity::error, Code::missing_pkl,
                    "No valid PKL found in DCP", dcp_dir});
    }
    ...
    for (const auto& asset : assetmap->assets) {
        ...
        if (cpl->reels.empty()) {
            result.add({Severity::error, Code::cpl_missing_reel,
                        "CPL has no reels", full_path});
        }
        ...
        for (const auto& reel : cpl->reels) {
            ...
            if (reel.sound.duration > 0 && reel.sound.duration != reel.picture.duration) {
                result.add({Severity::warning, Code::cpl_mismatched_durations,
                            "Sound duration differs from picture duration in reel " + reel.id,
                            full_path});
            }
        }
    ...
    if (!found_cpl) {
        result.add({Severity::error, Code::missing_cpl,
                    "No valid CPL found in DCP", dcp_dir});
    }
    ...
    if (opts.check_picture_details) {
        for (const auto& asset : assetmap->assets) {
            ...
            if (!mxf.valid) {
                result.add({Severity::error, Code::mxf_unreadable,
                            "Invalid MXF file: " + mxf.error, full_path});
                continue;
            }
            ...
            if (mxf.picture) {
                ...
                if (pic.width > 0 && pic.height > 0) {
                    bool valid_res =
                        (pic.width == 2048 && pic.height == 1080) ||
                    ...
                    if (!valid_res && opts.strict_smpte) {
                        result.add({Severity::warning, Code::picture_invalid_resolution,
                                    "Non-standard picture resolution: " +
                                    std::to_string(pic.width) + "x" + std::to_string(pic.height),
                                    full_path});
                    }
                }
                ...
                if (pic.frame_count > 0 && pic.frame_rate_num > 0 && pic.width > 0) {
                    auto bitrate_notes = check_j2k_bitrate(
                        full_path, pic.frame_count,
                        pic.frame_rate_num, pic.frame_rate_den,
            ...
            if (mxf.sound) {
                ...
                if (snd.sample_rate > 0 && snd.sample_rate != 48000 && snd.sample_rate != 96000) {
                    result.add({Severity::warning, Code::sound_invalid_sample_rate,
                                "Non-standard audio sample rate: " +
                                std::to_string(snd.sample_rate) + " Hz", full_path});
                }
                if (snd.channels > 0 && snd.channels > 16) {
                    result.add({Severity::warning, Code::sound_invalid_channel_count,
                                "Excessive audio channel count: " +
                                std::to_string(snd.channels), full_path});
                }
            }
        }
    }
    ...
    for (const auto& asset : assetmap->assets) {
        ...
        if (ext == ".xml") {
            ...
            if (fname.find("sub") != std::string::npos ||
                fname.find("Sub") != std::string::npos ||
                fname.find("SUB") != std::string::npos ||
                fname.find("timed") != std::string::npos) {
