    ...
    for (const auto& note : notes) {
        ...
        case Code::smpte_naming_violation:
            if (note.message.find("ASSETMAP") != std::string::npos) {
                suggestions.push_back(FixSuggestion{
                    Code::smpte_naming_violation,
                    "Rename ASSETMAP to ASSETMAP.xml for BV2.1 compliance",
                    "mv ASSETMAP ASSETMAP.xml",
                    true
                });
                ...
                suggestions.push_back(FixSuggestion{
                    Code::smpte_naming_violation,
                    "Rename PKL file to have .xml extension",
                    "",
        ...
        case Code::pkl_hash_mismatch:
            suggestions.push_back(FixSuggestion{
                ...
                "Regenerate PKL hashes (file may have been modified after packaging)",
                "",
                false
            });
            break;
        ...
        case Code::missing_required_element:
            if (note.message.find("ContentVersion") != std::string::npos) {
                suggestions.push_back(FixSuggestion{
                    ...
                    "Add ContentVersion element to CPL (required for BV2.1)",
                    "",
                    false
                });
                ...
                suggestions.push_back(FixSuggestion{
                    Code::missing_required_element,
                    "Add MainMarkers to first reel in CPL (FFOC, LFOC at minimum for BV2.1)",
                    "",
                    false
                });
            }
        ...
        case Code::isdcf_naming_violation:
            suggestions.push_back(FixSuggestion{
                Code::isdcf_naming_violation,
                "Rename content title to follow ISDCF naming convention: "
                "Title_ContentType_AspectRatio_Language_Territory_AudioType_Resolution_Studio_Date_Facility_Standard",
                "",
                false
            });
            break;
        ...
        case Code::sound_invalid_channel_count:
            if (note.message.find("MCA") != std::string::npos) {
                suggestions.push_back(FixSuggestion{
                    ...
                    "Add MCA (Multi-Channel Audio) labeling metadata to sound MXF",
                    "",
                    false
                });
            }
        ...
        case Code::marker_missing:
            suggestions.push_back(FixSuggestion{
                Code::marker_missing,
                "Add required markers to CPL (FFOC=first frame, LFOC=last frame, FFMC, LFMC for features)",
                "",
                false
            });
            break;
